...
if (!apiKey) {
  throw new Error(
    "Gemini API key is not configured. Please set VITE_API_KEY in your environment variables."
  );
}
  ...
  items: {
    ...
    properties: {
      ...
      correctOptionIndices: {
        ...
        description:
          "Zero-based indices of the correct options in the options array",
      },
...
export const generateQuizFromPDF = async (
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const questions: Question[] = rawData.map((item: any, index: number) => {
        const options: Option[] = item.options.map(
        ...
        const correctOptionIds = item.correctOptionIndices
  ...
  throw new Error(
    "PDF解析或题目生成失败: " + (lastError?.message || "Unknown error")
  );
};
